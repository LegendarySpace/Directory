<?php
    ...
    function generateID($size = 12) {
        ...
        // Replace with autogenerated UUID
        $id = '';
        ...
        if(!empty($purpose)) {
            ...
            switch($purpose) {
                ...
                case 'update':
                    ...
                    switch ($item) {
                        case 'user':
                            $typeArray = array("username"=>"Username","password"=>"Password","first name"=>"FirstName","last name"=>"LastName");
                            $t = $typeArray[$type];
                            $sql = "UPDATE `Users` SET `{$type}`={$value} WHERE AccountID={$userID}";
                            break;
                        case 'tower':
                            $typeArray = array("name"=>"Name","location"=>"Location","management company"=>"ManagementCompany",
                                "number"=>"ManagementContact","email"=>"ManagementContactEmail","image"=>"ImageURL","details"=>"Details");
                            $t = $typeArray[$type];
                            $sql = "UPDATE `Towers` SET `{$type}`={$value} WHERE AccountID={$userID}";
                            break;
                        case 'company':
                            $typeArray = array("name"=>"Name","suites"=>"Suite","reception"=>"Reception","number"=>"ContactNumber",
                                "email"=>"ContactEmail","slogan"=>"Slogan","details"=>"Details","image"=>"ImageURL");
                            $t = $typeArray[$type];
                            $sql = "UPDATE `Companies` SET `{$t}`={$value} WHERE AccountID={$userID}";
                            break;
                        case 'event':
                            $typeArray = array("name"=>"Name","company"=>"CompanyID","tower"=>"TowerID","host"=>"Host",
                                "slogan"=>"Slogan","location"=>"Location","details"=>"Details");
                            $t = $typeArray[$type];
                            $sql = "UPDATE `Events` SET `{$t}`={$value} WHERE AccountID={$userID}";
                            break;
                        case 'employee':
                            $typeArray = array("first name"=>"FirstName", "last name"=>"LastName", "title"=>"Title", "phone"=>"Phone",
                                "email"=>"Email", "address"=>"Address", "public"=>"Public", "image"=>"ImageURL");
                            $t = $typeArray[$type];
                            $sql = "UPDATE `Employees` SET `{$t}`={$value} WHERE AccountID={$userID}";
                            break;
                        default:
